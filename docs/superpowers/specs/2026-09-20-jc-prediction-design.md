...
## 9.1 α 校准记录（2026-09-20，实现后首跑）

**原设定的 α（trend 0.5 / cross 0.2 / vol 阈值 0.15）会让路径在决策层退化。**
实测当期 26 场：

| 信号 | 中位 | 最大 |
|---|---|---|
| 概率首二名差距 | 0.24 | 0.68 |
| 归一化漂移 `nd` | **-0.008**（赔率只变 0.8%） | 0.077 |
| 波动 `vol` | 0.013 | 0.074 |

- 以 α=0.5 计，**5 条路径 pick 完全一致（分歧 0/26）** —— 记录再多天也分不出高下，
  「用数据淘汰弱路径」的设计目的落空。实测要翻转 argmax 需 α≈1.4–4.9。
- `vol` 最大仅 0.074，原阈值 0.15 让 `stable` **几乎从不触发**（等于没有过滤）。

**校准后**（trend 2.0 / cross 0.5 / vol 阈值 0.04），首跑分歧率：

| 路径 | 分歧 | 不出手 |
|---|---|---|
| `trend` | 12/130 (9.2%) | 0 |
| `blend` | 9/130 (6.9%) | 0 |
| `stable` | 0 | 41/130 (31.5%) |
| `cross` | 0/26 | 0 |

**`cross` 的分歧率天然为 0，这不是缺陷**：`support` 归一化后若跨玩法推导
（`hafu`/`crs`）与 `had` 的排序一致，argmax 不会改变。而那些玩法同样是市场赔率，
反映的是**同一个市场共识**，排序通常一致。因此 `cross` 的价值不在"选谁"，
而在"**有多确信**"—— 验证时应单独统计「高印证度 vs 低印证度场次的命中率」，
而不是看 pick 分歧。

**务必记住**：α 调大**不代表趋势信号更强了**。它只是为了让路径能产生分歧、
从而产出可对比的数据。当前赔率漂移本身极微弱（0.8%），`trend` 大概率会输给
`market` —— 而这个结论正是我们要的数据。原始信号已按选项落库，将来可离线
重新拟合，不必重跑采集。
